...
def create_combined_dashboard(
    collab_df: pd.DataFrame, df: pd.DataFrame, df_counts: pd.DataFrame
):
    ...
    app.layout = html.Div(
        [
            html.H1("Diagrama de Arcos  e  Gráfico de Linhas — Linking"),
            ...
            html.Div(
                dcc.Dropdown(
                    ...
                    options=[
                        {"label": str(p), "value": p}
                        for p in sorted(df["researcher_name"].unique())
                    ],
                    value=[],
                    multi=True,
                    style={"width": "100%"},
            ...
            html.Div(
                [
                    html.Label(
                        "selecione um ou mais pesquisadores (ou clique no diagrama):"
                    ),
                    dcc.Graph(
                        id="grafico-linhas", style={"width": "100%", "height": "700px"}
                    ),
                ],
                style={"width": "100%", "marginBottom": "40px"},
            ),
    ...
    @app.callback(
        Output("researcher-dropdown", "value"), Input("selected-researchers", "data")
    )
    ...
    @app.callback(
        Output("grafico-linhas", "figure"), Input("selected-researchers", "data")
    )
    def update_line_graph(selected):
        if not selected:
            return go.Figure().update_layout(
                title={
                    "text": "Selecione ao menos um pesquisador",
                    "y": 0.9,
                    "x": 0.5,
                    "xanchor": "center",
                }
        ...
        for i, (_, row) in enumerate(df_show.iterrows()):
            ...
            df_sel = df[
                (df["researcher_name"] == researcher) & (df["article_year"] == year)
            ]
            blob_text = (
                "<br>".join(df_sel["article_title"].tolist())
                if not df_sel.empty
                else "Sem artigos"
            )
            key = (year, count)
            ...
            if len(value) == 1:
                ...
                for researcher, text in value:
                    blob_text += (
                        ...
                        + researcher
                        + "  "
                        + (square_char * (max(1, 20 - len(researcher))))
                        + "</b>"
                        + "<br>"
                    )
    ...
    @app.callback(
        Output("arc-diagram", "figure"), Input("selected-researchers", "data")
    )
    def update_arc_on_selection(selected):
        ...
        for trace_idx in arc_traces:
            ...
            opacity = (
                1.0
                if any(idx in sel_indices for idx in connected)
                else (0.1 if sel_indices else 1.0)
            )
