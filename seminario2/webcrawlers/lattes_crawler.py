...
DOCENTES_PATH = "docentes.csv"
DATA_PATH = "professores.pkl"  ## salva o backup
...
professores = recover_backup(
    DATA_PATH
)  ## relaciona cada professor como nome:(listaartigos,listapesquisa)
...
for i, (idx, (nusp, nome, link, pagina, link_lattes)) in enumerate(df.iterrows()):
    try:
        ...
        if (
            "Projetos de pesquisa" not in div2corename.keys()
        ):  ## caso o prof não tenha cadastrado como produções
            print(" " * 4, "Campo de projetos de pesquisa não encontrado!!")
        else:
            pesquisasdiv = div2corename["Projetos de pesquisa"]
            pesquisasdiv = pesquisasdiv.find_elements(By.XPATH, "./*")[
                -2
            ]  ## vai até o penúltimo filho
            pesquisasdiv = pesquisasdiv.find_elements(
                By.XPATH, "./*"
            )  ## pega todos os filhos dessa div
            ...
            for element in pesquisasdiv:
                ...
                if class_name == "a":
                    if pesquisa_nome is not None:
                        ...
                        pesquisas.append(
                            Pesquisa(pesquisa_nome, pesquisa_data, pesquisa_descricao)
                        )
                        pesquisa_nome = (
                            None  ## variavel auxiliar pra guardar o nome da pesquisa
                        )
                        pesquisa_data = None
                        pesquisa_descricao = None
                ...
                if step == 1:  ## guarda a data
                    pesquisa_data = element.find_element(
                        By.XPATH, "./*"
                    ).text  ## pega o texto do primeiro filho

                if step == 2:  ## bglh que guarda o nome da pesquisa
                    pesquisa_nome = element.find_element(
                        By.XPATH, "./*"
                    ).text  ## pega o texto do primeiro filho

                if step == 4:  ## bglh que tem a descrição
                    pesquisa_descricao = element.find_element(
                        By.XPATH, "./*"
                    ).text  ## pega o texto do primeiro filho
        ...
        if "Produções" not in div2corename.keys():
            ...
            artigosdiv = driver.find_element(
                By.ID, "artigos-completos"
            )  ## lista o div que guarda os artigos completos
            artigos = artigosdiv.find_elements(
                By.CLASS_NAME, "artigo-completo"
        ...
        print(f"Erro no professor {nome}. Erro: ", e)
        print("\n" * 5)
